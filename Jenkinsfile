            ...
            steps {
                script {
                    ...
                    withCredentials([azureServicePrincipal('sp-iac-azure')]){
                        ...
                        sh "ansible-playbook site.yml -i hosts -e COMPONENT=" + env.TYPE_TASK + " -e subscription_id=${AZURE_SUBSCRIPTION_ID} -e client_id=${AZURE_CLIENT_ID} -e secret_id=${AZURE_CLIENT_SECRET} -e tenant_id=${AZURE_TENANT_ID} -v"
